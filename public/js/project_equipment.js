...
$('#addsignatory').click(function () {
      ...
      $("#signatory_table")
        .append('<tr><td><input type="text" name="signname[]" value="' + "name" + '"  style="border: white; width: 100%;"/>' + '</td>' + 
						'<td>' + '<input type="text" name="signposition[]" value="' + "position" + '"  style="border: white; width: 100%;"/>'  + '</td>' +
	     				'<td> <a  class="btn btn-danger btn-xs" onclick="delete_signatory( this )"><i class="fa fa-trash-o"></i> </a></td></tr>');

  });

// Remove signatory to HTML table
function delete_signatory(r) {
  var signatory_table = document.getElementById("signatory_table").rows.length - 1;
  var i = r.parentNode.parentNode.rowIndex;
  if (signatory_table == 1) {
    document.getElementById("submit").disabled = true;
  }
  document.getElementById("signatory_table").deleteRow(i);
}
